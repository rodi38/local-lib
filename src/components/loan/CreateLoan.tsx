...
function CreateLoan() {
  ...
  const handleSearch = async (value: string, context: string) => {
    try {
      const response = await api.get(
        `/${context}?search=${value}&page=0&size=1000`
      );
      if (context === "student") {
  ...
  const debouncedSearch = _.debounce(
    (value, context) => handleSearch(value, context),
    300
  );
        ...
        <Row gutter={16}>
          <Col span={12}>
            <Form.Item
              ...
              name="studentId"
              rules={[
                { required: true, message: "Por favor insira o estudante!" },
              ]}
            >
              <AutoComplete
                style={{ width: "100%" }}
              ...
              >
                {bookSearchResults.map((book: Book) => (
                  <Option key={book.id} value={book.id}>
                    {book.title} | Quantidade em estoque: {book.stockQuantity}
                  </Option>
                ))}
              </AutoComplete>
            </Form.Item>
